...
router.get('/:address/:chain', async function(req, res) {
  ...
  let balance = await alchemy.forNetwork(networksList[chainId]).core.getBalance(address, "latest")
  let nativeBalance = Utils.formatEther(balance, 'ether')
  let networkInfo = await alchemy.forNetwork(networksList[chainId]).core.getNetwork(address)
  let ens = false
  networkInfo.name == "homestead" ? ens = await alchemy.forNetwork(networksList[chainId]).core.lookupAddress(address) : false
  let blockHeight = await alchemy.forNetwork(networksList[chainId]).core.getBlockNumber()
  res.json({nativeBalance, ens: ens, network: networkInfo.name, chainId: networkInfo.chainId, blockHeight})
})
